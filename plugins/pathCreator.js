...
for (const i in iconMarker.move) {
  iconMarker[iconMarker.move[i]] = {};
  for (const u in iconMarker.type) {
    iconMarker[iconMarker.move[i]][iconMarker.type[u]] = new moveSet(
      `./data/assets/path/${iconMarker.move[i]}/${iconMarker.type[u]}.svg`,
      `${iconMarker.move[i]}-arrow`
    );
  }
}
...
function onMapClick() {
  actualMap = actualMap = $("#mapCoordinates")[0].innerText.split(", ")
    ? !loadingFile
    : actualMap;
  ...
  if ($(".fa-suitcase.toggled").length > 0) {
    if (!havenToggle) {
      if (!checkMaps(actualMap[0] + "," + actualMap[1], havenBagData)) {
        ...
        havenBagData.splice(
          getMapIndex(actualMap[0] + "," + actualMap[1], havenBagData),
          1
        );
      }
    } else {
    ...
    let deleteAll = function(array, index) {
      if (array[index] !== undefined) {
        ...
        if (array[index].bottom)
          if (array[index].bottom.icon)
            map.removeLayer(array[index].bottom.icon);
        if (array[index].left)
        ...
        if (array[index].havenbag)
          if (array[index].havenbag.icon)
            map.removeLayer(array[index].havenbag.icon);
    ...
    for (let key in bankData) {
      console.log(
        key +
          ...
          bankData[key].map +
          ", top : " +
          bankData[key].top.path +
          ", bottom : " +
          bankData[key].bottom.path +
          ", right : " +
          bankData[key].right.path +
          ", left : " +
          bankData[key].left.path +
          ", havenbag : " +
          bankData[key].havenbag.path
      );
    }
    ...
    for (let key in phenixData) {
      console.log(
        key +
          ...
          phenixData[key].map +
          ", top : " +
          phenixData[key].top.path +
          ", bottom : " +
          phenixData[key].bottom.path +
          ", right : " +
          phenixData[key].right.path +
          ", left : " +
          phenixData[key].left.path +
          ", havenbag : " +
          phenixData[key].havenbag.path
      );
    }
  } else {
    if (!checkMaps(actualMap, pathData)) {
    ...
    for (let key in pathData) {
      console.log(
        key +
          ...
          pathData[key].map +
          ", top : " +
          pathData[key].top.path +
          ", bottom : " +
          pathData[key].bottom.path +
          ", right : " +
          pathData[key].right.path +
          ", left : " +
          pathData[key].left.path +
          ", havenbag : " +
          pathData[key].havenbag.path +
          ", gather : " +
          pathData[key].gather +
          ", fight : " +
          pathData[key].fight
...
function deleteEmptyPath(array) {
  if (
    !array[getMapIndex(actualMap, array)].top.path &&
    !array[getMapIndex(actualMap, array)].bottom.path &&
    !array[getMapIndex(actualMap, array)].left.path &&
    !array[getMapIndex(actualMap, array)].right.path &&
    !array[getMapIndex(actualMap, array)].havenbag.path
  ) {
  ...
  if ($("." + className + ".toggled").length > 0) {
    if ($(".fa-leaf.toggled").length > 0) {
      if (iconType == "havenbag") {
        array[key][iconType].icon = L.marker(
          dofusXYToMapXY(actualMap[0], actualMap[1]),
          {
            interactive: true,
            zIndexOffset: 5000,
          ...
          .addTo(map)
          .bindPopup(
            "Téléportation au zaap map : " +
              havenBagData[havenBagData.length - 1].mapToGo +
              " | mapID : " +
              havenBagData[havenBagData.length - 1].mapidToGo
          );
      } else {
        array[key][iconType].icon = L.marker(
          dofusXYToMapXY(actualMap[0], actualMap[1]),
      ...
      if (iconType == "right" || iconType == "left") {
        array[key][iconType].icon = L.marker(
          dofusXYToMapXY(actualMap[0], actualMap[1] * 1 + 0.2),
          {
            interactive: false,
            zIndexOffset: 4500,
        ...
        array[key][iconType].icon = L.marker(
          dofusXYToMapXY(actualMap[0] * 1 + 0.1, actualMap[1]),
          {
            interactive: false,
            zIndexOffset: 4500,
            icon: icon[iconType].bank
  ...
  if (index !== undefined) {
    if (array.length !== 0) {
      if (array[index].havenbag.path)
        ...
        if (array[index].top.path) path = "top";
        if (array[index].bottom.path)
          path += (path === "" ? "" : "|") + "bottom";
        if (array[index].left.path) path += (path === "" ? "" : "|") + "left";
        if (array[index].right.path) path += (path === "" ? "" : "|") + "right";
      }
      return path;
    }
  }
...
function gatherFigh(index, array) {
  var path = "";
  if (array[index].gather) path = ", gather = true";
  if (array[index].fight) path += ", fight = true";
  return path;
}
...
$("#path-creator-bar").on("click", ".fa-save", function() {
  ...
  var txt =
    ...
    "\r\n\r\n" +
    "GATHER = {" +
    ($("#ressources-selecter").val() ? $("#ressources-selecter").val() : "") +
    "}\r\n" +
    "OPEN_BAGS = " +
    $("#open-bag-checkbox").is(":checked") +
    "\r\n" +
    "AUTO_DELETE = {" +
    $("#auto-delete-field").val() +
    "}\r\n\r\n" +
    "MAX_MONSTERS = " +
    $("#max-monster").val() +
    "\r\n" +
    ...
    "\r\n\r\n" +
    "FORBIDDEN_MONSTERS = {" +
    $("#forbidden-ennemy-field").val() +
    "}\r\n" +
    "FORCE_MONSTERS = {" +
    $("#mendatory-ennemy-field").val() +
    "}\r\n\r\n" +
    "" +
    "function hiboux()\r\n" +
    "\tnpc:npc(522,3)\r\n" +
  ...
  if (pathLinearData.length !== 0) {
    ...
    for (let key in pathLinearData) {
      txt +=
        '\t{map = "' +
        pathLinearData[key].map.replace(" ", "") +
        '", changeMap = "' +
        getLinearAction(1, key) +
        '"' +
        (pathLinearData[key].index >= 2
          ? ', nextMap = "' + getLinearAction(2, key) + '", doNextMap = "no"'
          : "") +
        (pathLinearData[key].index >= 3
          ? ', thirdMap = "' + getLinearAction(3, key) + '", doThirdMap = "no"'
          : "") +
        (pathLinearData[key].index >= 4
          ? ', fourthMap = "' +
            getLinearAction(4, key) +
            '", doFourthMap = "no"'
          : "") +
  ...
  for (let key in bankData) {
    txt +=
      '\t\t{map = "' +
      bankData[key].map.replace(" ", "") +
      '", path = "' +
      pathGenerator(key, bankData) +
      '"},\r\n';
  }
  txt +=
  ...
  for (let key in phenixData) {
    if (checkPhenixPos(phenixData[key].map.replace(" ", ""))[0]) {
      txt +=
        '\t\t{map = "' +
        phenixPos[checkPhenixPos(phenixData[key].map.replace(" ", ""))[1]].map +
        '", phenix = "' +
        phenixPos[checkPhenixPos(phenixData[key].map.replace(" ", ""))[1]]
          .cellid +
        '", path = "' +
        pathGenerator(key, phenixData) +
        '"},\r\n';
    } else {
      txt +=
        '\t\t{map = "' +
        phenixData[key].map.replace(" ", "") +
        '", path = "' +
        pathGenerator(key, phenixData) +
        '"},\r\n';
    }
        ...
        if (/(-|)[0-9]+?,\s(-|)[0-9]+?/g.exec(actualMap)) {
          let loadAddLinearAction = function(param) {
            ...
            if (matche !== null) {
              ...
              if (matche[1] == "bottom")
                $(".fa-arrow-down").addClass("toggled");
              if (matche[1] == "right")
                $(".fa-arrow-right").addClass("toggled");
            }
...
$("#havenBagSetting").on("click", ".btn-primary", function() {
  ...
  if (
    $(".fa-arrows-alt.toggled").length > 0 ||
    $(".fa-leaf.toggled").length > 0 ||
    $(".fa-shield-alt.toggled").length > 0
  )
    havenBagData.push({
          ...
          if (pathLinearData[index].index == 4) {
            if ("fr") {
              $("#mapCoordinates").append(
                "<span><br />Premiére action sur la map : " +
                  linearActualItem(index, 1) +
                  "<br />Deuxiéme action sur la map : " +
                  linearActualItem(index, 2) +
                  "<br />Troisiéme action sur la map : " +
                  linearActualItem(index, 3) +
                  "<br />Quatrième action sur la map : " +
                  linearActualItem(index, 4) +
                  "</span>"
              );
            }
            // } else if (GM_getValue('language') == 'en') {
...
$("#path-creator-bar").on(
  "click",
  ".fa-arrows-alt, .fa-shield-alt, .fa-leaf, .fa-university, .fa-fire",
  function() {
    $(".path-type-selecter .toggled").removeClass("toggled");
    $(this).toggleClass("toggled");
    if ($(".fa-fire.toggled").length > 0) {
      for (let key in phenixPos) {
        if (phenixPos[key].marker) map.removeLayer(phenixPos[key].marker);
        phenixPos[key].marker = undefined;
      }
      for (let key in phenixPos) {
        phenixPos[key].marker = L.marker(
          dofusXYToMapXY(
            phenixPos[key].map.split(",")[0] * 1 + 0.5,
            phenixPos[key].map.split(",")[1] * 1 + 0.5
          ),
          {
            interactive: true,
            zIndexOffset: 5000,
    ...
    if ($(".fa-university.toggled").length > 0) {
      for (let key in bankPos) {
        if (bankPos[key].marker) map.removeLayer(bankPos[key].marker);
        bankPos[key].marker = undefined;
      }
      for (let key in bankPos) {
        bankPos[key].marker = L.marker(
          dofusXYToMapXY(
            bankPos[key].map.split(",")[0] * 1 + 0.5,
            bankPos[key].map.split(",")[1] * 1 + 0.5
          ),
          {
            interactive: true,
            zIndexOffset: 5000,
...
$("#pathSetting").on("change", "#ressources-selecter", function() {
  ...
  $("#ressources-selecter option:selected").each(function() {
    $("#ressources-to-gather .ressources-selected-summary").append(
      '<button class="btn btn-success" style="margin: 2.5px;">' +
        $(this).text() +
        "</button>"
    );
  });
